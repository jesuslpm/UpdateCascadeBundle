...
Q. How do I configure a denormaliced reference to be updated automatically when the referenced entity changes?
A. 
...
A.
	The in progress cascade operation is cancelled, and a new cascade operation is started.

Q. What happens then the server shutsdown or crashes in the middle of a cascade operation?
A.
	When the server shutsdown all in progress cascade operations are cancelled.
	When the server crashes I can't do nothing, but there is no problem at all.
